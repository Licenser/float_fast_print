...
fn f32_test_exhaustive_roundtrip() {
    ...
    for bits in 0u32..=std::u32::MAX {
        ...
        if 0 == (bits % 1000000) {
            println!(
                "f32 exhaustive {}% complete",
                (bits as f64 * 100.0 / (std::u32::MAX as f64)) as f32
            );
        }
        ...
        if !(parsed.is_nan() && float.is_nan()) {
            if parsed != float {
                println!(
                    "for bits: {} float {} => \"{}\" != {} (parsed)",
                    bits, float, float_str, parsed
                );
            }
...
fn f64_test_many_roundtrip() {
    ...
    for progress in 0u32..=std::u32::MAX {
        ...
        if 0 == (bits % 1000000) {
            println!(
                "f64 strong testing {}% complete",
                (progress as f64 * 100.0 / (std::u32::MAX as f64)) as f64
            );
        }
